...
export const createCapsuleValidation = [
  ...
  body('extra').optional({ checkFalsy: true, nullable: true }).isString().withMessage('توضیحات اضافی شما نامعتبر می‌باشد').isLength({ min: 32, max: 500 }).withMessage('توضیحات اضافی شما باید حداقل ۳۲ و حداکثر ۵۰۰ کاراکتر باشد').trim().bail(),
];

export const editCapsuleValidation = [
  body('title').optional({ checkFalsy: true, nullable: true }).isString().withMessage('عنوان شما نامعتبر می‌باشد').isLength({ min: 4, max: 16 }).withMessage('عنوان شما باید حداقل ۴ و حداکثر ۱۶ کاراکتر باشد').trim().bail(),

  body('description').optional({ checkFalsy: true, nullable: true }).isString().withMessage('توضیحات شما نامعتبر می‌باشد').isLength({ min: 32, max: 500 }).withMessage('توضیحات شما باید حداقل ۳۲ و حداکثر ۵۰۰ کاراکتر باشد').trim().bail(),
  ...
  body('access')
    ...
    .custom((access) => {
      ...
      if (access.visibility === 'public') {
        if (access.lock && access.lock !== 'none') throw new Error('برای visibility=public فقط lock=none مجاز است');
        if (access.unlockAt) throw new Error('مقدار unlockAt نباید برای حالت عمومی ارسال شود');
      }
      ...
      if (access.lock === 'timed') {
        ...
        if (!access.unlockAt) throw new Error('برای lock=timed مقدار unlockAt الزامی است');
        const d = new Date(access.unlockAt);
        if (isNaN(d.getTime()) || d.getTime() <= Date.now()) throw new Error('unlockAt باید تاریخ معتبر آینده باشد');
      }
      return true;
    }),

  body('color').optional({ checkFalsy: true, nullable: true }).isString().withMessage('رنگ باید رشته باشد'),
